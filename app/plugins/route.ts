...
import { useAppTransition, type AppTransitionCloseHandle } from '~/composables/useAppTransition';
...
export default defineNuxtPlugin(() => {
    ...
    let tlClose: AppTransitionCloseHandle | undefined;
    let isFirstPageLoaded = false;
    let pathToTemp = '';
    let disabledTransition = false;
    let _to: RouteLocationNormalized | null = null;
    let _toHash = '';
    let _toQuery: RouteLocationNormalized['query'] = {};
     ...
     */
    const checkDisabledTransition = (to: RouteLocationNormalized, from: RouteLocationNormalized): boolean => {
        try {
     ...
     * 路由切換前處理
     */
    router.beforeEach(async (to: RouteLocationNormalized, from: RouteLocationNormalized, next: () => void) => {
        // 如果是靜態資源路徑，直接跳過路由處理
        ...
        if (import.meta.client) {
            ...
            if (from.path !== to.path) {
                if (import.meta.dev) console.warn('🔄 [Route] 路由切換開始', { from: from.path, to: to.path });
                ...
                if (import.meta.dev) console.warn('🔄 [Route] 已設置 SET_PAGE_TRANSITION_FINISHED(false)');
     ...
     * 應用程式錯誤處理
     */
    nuxtApp.hook('app:error', async (_err) => {
        if (import.meta.client) {
            await nextTick();
            resetHtmlClass();
    ...
    nuxtApp.hook('page:widgets:complete', async () => {
        ...
        if (isFirstPageLoaded) {
            ...
            if (!disabledTransition) {
                ...
                tlClose
                    ?.eventCallback('onUpdate', function (this: AppTransitionCloseHandle) {
                        // ⭐ 使用 this.progress() 檢查進度（與 LRC 一致）
                        // this 綁定為 eventCallback 呼叫時傳入的 returnObj（見 useAppTransition.ts）
                        if ((this.progress() >= storeGlobal.PageFinishedBeforeEndProgress || this.totalDuration() <= 0) && !isTriggerFinished) {
