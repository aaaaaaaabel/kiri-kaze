    ...
    <div class="grid_list" :class="[`grid_list${gridClass}`, { 'grid_list_on': isHovered, 'grid_list_off': isHoveredOff }, viewModeClass]">
        ...
        >
            <div ref="numberRef" class="grid_number">
                <span/>
                <p class="tracking en0">{{ index + 1 }}</p>
            </div>
            <div ref="imageRef" class="grid_img">
                ...
                <!-- 第二個 div：顯示 cover（grid_image 模式顯示） -->
                <div>
                    <div>
                        <img 
                            :data-src="coverUrl" 
...
// PC hover 效果（使用 grid_list_on class）
@include pc {
    .grid_img > div > div {
        @include transition(transform, 0.25s);
    }
    ...
    // 離開 hover 時觸發抖動動畫（grid_list_off）
    .grid_item .grid_list_off .grid_img div div img {
        animation: grid-off-shake 2s ease-in-out forwards;
    }
...
// 離開 hover 時的抖動動畫（grid_off1）
@keyframes grid-off-shake {
    0%,
    100% {
        transform: rotate(0);
        transform-origin: left bottom;
