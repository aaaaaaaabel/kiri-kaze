...
- `fossils`/`projects` 單筆查詢端點補上 `isPublic` 檢查（broken-access-control 修復），commit `670ca94`
- 刪除確認沒有呼叫者的 mock composable（`useMockFossils.ts`/`useMockSpecies.ts`/`useMockProjects.ts`/`useMockEvents.ts`），對應文件已更新，commit `d69b110`
- 本機資料夾改名：使用者已在 session 外自行執行完成，目前路徑是 `/Users/abel/project/lacunae`（跟原計畫的 `lacunae-studio` 不同，但同樣脫離了 `kiri-kaze`；GitHub/Vercel remote 本來就沒受影響）
